...
def txt2xls(path_name):
    ...
    for filename in onlyfiles:
        ...
        header_exel = ["Num_Fold", "Perc_Oversampling","Right_index","Num_Classifiers","Accuracy","Prec_T0","Rec_T0","F1_T0","Prec_T1","Rec_T1","F1_T1","TP", "TN", "FP", "FN","Run_Time", "N_Cluster", "Class_for_Clustering"]
        ...
        sh.write(19, c1, "Class_for_Clustering:")
        sh.write(19, c2, "Indica le classi utilizzate in fase di clustering")
        ...
        with open(path) as f:
            ...
            find_conf_mtr = 0
            find_stat_mtr = 0
            class_for_clustering="A, B"
            print_line = False
            index_excel = 0
            cv_iteration=0
            for line in lines:

                if line.__contains__("Classes used for clustering: "):
                    l_split = line.split(": ")
                    class_for_clustering = l_split[1]
                ...
                if print_line == True:
                    ...
                    sh.write(index_excel, 17, class_for_clustering)
                    if float(f12)>=float(max_f1):
                        ...
                        max_class_for_clustering= class_for_clustering


        f.close()
        sh.write(20, c1, "Risultato migliore", style=style)
        sh.write(21, c1, "Riga " + str(max_row)+ ". Si ha un F1-Score="+str(int(max_f1*100))+"%.")
        sh.write(22, c1, "Prametri utilizzati: n_right="+str(max_n_right)+", n_clf="+str(max_n_clf)+", percentage=" + str(max_perc) +" e n_cluster=" + str(max_n_cluster) + " (" + str(max_class_for_clustering) +")")
