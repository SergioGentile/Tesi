...
def read_csv_restricted(path_name, target=''):
    """
read_csv legge il dataset specificato come parametro e separa header, dataset per il training e il target
    :param path_name: path da cui leggere il dataset
    :param target: specifica il livello di rischio su cui si vuole lavorare (uno tra NNC, NCD, n_caused_claim, cost_caused_claim)
    ...
    for column in header:
        ...
        if column == 'Anno':
            anno_index = index
        if column == 'DFINE_VLDT_GRZI':
            DFINE_VLDT_GRZI_index = index
        if column == 'DINZI_VLDT_GRZI':
            DINZI_VLDT_GRZI_index = index
        if column == 'NNC':
            NNC_index = index
        if column == 'NCD':
            NCD_index = index
        ...
        if column == 'NPLZA':
            NPLZA_index = index
        if column == 'provincia_new':
            provincia_new_index = index
        if column == 'area_territoriale_tarif_new':
            area_territoriale_tarif_new_index = index
        if column == 'regione_tarif_new':
            regione_tarif_new_index = index
        if column == 'istat_new':
            istat_new_index = index
        if column == 'CAP_NEW':
            CAP_NEW_index = index
        if column=='data_ultima_voltura':
            data_ultima_voltura_index = index
        if column=='professione':
            professione_index = index
        if column == '':
            empty_index = index
        index = index+1
    ...
    #Rimuovo anche una colonna vuota presente a fine file (Errore nella discretizzazione?)
    to_delete = []
    to_delete.append(professione_index)
    to_delete.append(data_ultima_voltura_index)
    to_delete.append(CAP_NEW_index)
    to_delete.append(istat_new_index)
    to_delete.append(regione_tarif_new_index)
    to_delete.append(area_territoriale_tarif_new_index)
    to_delete.append(provincia_new_index)
    to_delete.append(DINZI_VLDT_GRZI_index)
    to_delete.append(DFINE_VLDT_GRZI_index)
    to_delete.append(Settima_index)
    to_delete.append(anno_index)
    to_delete.append(NPLZA_index)
    to_delete.append(NNC_index)
    to_delete.append(NCD_index)
    to_delete.append(n_caused_claim_index)
    ...
    del header[NCD_index]
    del header[NNC_index]
    del header[data_ultima_voltura_index]


    del header[professione_index]
    del header[CAP_NEW_index]
    del header[istat_new_index]
    del header[regione_tarif_new_index]
    del header[area_territoriale_tarif_new_index]
    del header[provincia_new_index]
    del header[NPLZA_index]
    del header[DINZI_VLDT_GRZI_index]
    del header[DINZI_VLDT_GRZI_index]
    del header[DFINE_VLDT_GRZI_index]
    del header[anno_index]
    del header[Settima_index]
