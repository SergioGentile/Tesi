...
def undersampling_prediction(model, clf_name, dataset, target, dataset_name, labels=None, risk_level=None, percentage_A=50):
...
Successivamente viene applicata la LeaveOneOut validation per analizzare la classificazione.
    ...
    for train_index, test_index in loo.split(dataset):
        ...
        target_predicted.append(1)#clf.predict(X_test)[0])
        y.append(y_test)
        if first_time == True:
            first_time=False
            end_clf = time.time()
    ...
    with open(filename_statistics, "a") as myfile:
        ...
        myfile.write("Algorithm name: " + str(clf_name) + "\n")
        print("Algorithm name: " + str(clf_name))
